from pathlib import Path

p=Path("app/autopilot_runner.py")
text=p.read_text(encoding="utf-8")

anchor='run_step("meta_live_campaign_builder","python -m app.meta_live_campaign_builder")'

replacement='''
run_step("meta_live_campaign_builder","python -m app.meta_live_campaign_builder")
run_step("google_live_campaign_builder","python -m app.google_live_campaign_builder")'''

if 'google_live_campaign_builder' not in text:
    text=text.replace(anchor,replacement)

p.write_text(text,encoding="utf-8")

print("google_live_campaign_builder added to autopilot")
