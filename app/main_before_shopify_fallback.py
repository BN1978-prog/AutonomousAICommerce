...
admin = os.getenv("SHOPIFY_ADMIN_TOKEN","").strip()
access = os.getenv("SHOPIFY_ACCESS_TOKEN","").strip()
...
# --- Meta AI recommendation endpoint ---
@app.get("/api/meta/ai-recommendation")
def meta_ai_recommendation():
    from app.db import SessionLocal
    from app.models import MetaEvent
    ...
    if purchases >= 1 and purchase_rate >= 5:
        ...
        action = "PAUSE_OR_FIX_FUNNEL"
        reason = "Traffic is coming but no AddToCart. Check product page, price, offer, or targeting."
    else:
